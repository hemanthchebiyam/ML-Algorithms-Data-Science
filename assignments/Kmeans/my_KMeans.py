...
class my_KMeans:
    ...
    def initiate(self, X):
        ...
        if self.init == "random":
            ...
            while len(cluster_centers) < self.n_clusters:
                dists = np.array([min([self.dist(x, center) for center in cluster_centers]) for x in X])
    ...
    def fit_once(self, X):
        ...
        for i in range(self.max_iter+1):
            ...
            cluster_centers = [np.mean(cluster, axis=0) if cluster else center for cluster, center in zip(clusters, cluster_centers)]
